...
// });
  ...
  <div class="dealCard__priceCol">
    <div class="dealCard__priceLabel">
      Total for [[+days]] [[+days:is=`1`:then=`day`:else=`days`]]
    </div>

    <div class="dealCard__priceValue">
      $ [[+price:isnot=``:then=`[[+price]]`:else=`Rate not available`]]
    </div>
...
/**
 * Save POST search values into session
 */
    ...
    function calculateRentalPrice($modx, $carCode, $pickupDate, $dropoffDate) {
        ...
        for ($d = 1; $d <= $days; $d++) {
            if (isset($rates[$d])) {
                ...
                // use the last available rate for extra days
                $total += $lastRate;
            } else {
                // missing duration inside the range 1..maxDuration
                return \'\';
            }
        }
...
foreach ($rows as $row) {
    ...
    $row[\'calculated_price\'] = calculateRentalPrice(
        $modx,
        $row[\'car_code\'] ?? \'\',
        $pickupDate,
        $dropoffDate
