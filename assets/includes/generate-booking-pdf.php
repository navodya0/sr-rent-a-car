...
if (!function_exists('generateBookingPdfFile')) {
    if (!function_exists('sendBookingPdfToCustomer')) {
        function sendBookingPdfToCustomer(array $bookingData, string $pdfPathFs): bool
        {
            ...
            try {
                ...
                $mail->SMTPAuth   = true;
                $mail->Username   = 'terminalprimesrilanka@gmail.com';
                $mail->Password   = 'bxqr vdre kwng sjql'; 
                $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // try STARTTLS if needed
                $mail->Port       = 587;

                // EMAIL SETTINGS
                $mail->setFrom('terminalprimesrilanka@gmail.com', 'SR Rent A Car');
                $mail->addAddress($customerEmail, $customerName);
                $mail->addReplyTo('terminalprimesrilanka@gmail.com', 'SR Rent A Car');
    ...
    function generateBookingPdfFile(array $bookingData): ?string
    {
        ...
        $couponCode             = (string)($bookingData['coupon_code'] ?? '');
        $couponDiscountAmount   = (float)($bookingData['coupon_discount_amount'] ?? 0);
            ...
            <div class="total-box">
                <table>
                    ...
                    ' . (
                        ...
                        ? '<tr>
                                <td class="muted">Coupon Discount</td>
                                <td class="num" style="text-align:right; font-weight:700; color:#b91c1c;">-' . $h($fmt($couponDiscountAmount)) . '</td>
                        </tr>'
                        : ''
                    ) . '
                    <tr>
